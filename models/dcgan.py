...
class DCGAN_G_C(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        super(DCGAN_G_C, self).__init__()
        self.ngpu = ngpu
        assert isize % 16 == 0, "isize has to be a multiple of 16"
    ...
    def forward(self, input, noise_condition):
        if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu > 1:
            ...
            output = nn.parallel.data_parallel(self.main, combined , range(self.ngpu))
        else: 
            combined = torch.cat((input, noise_condition),1)
...
class DCGAN_D_C(nn.Module):
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0):
        super(DCGAN_D_C, self).__init__()
        self.ngpu = ngpu
        assert isize % 16 == 0, "isize has to be a multiple of 16"
